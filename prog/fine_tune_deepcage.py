...
class Mycallback(Callback)            :
    ...
    def on_epoch_end(self,epoch,logs={}):
        ...
        model.save('%s/checkpoint/model_at_epoch_%d2.h5'%(DPATH,epoch))
...
def model_training(data_queue,batch_size,epochs):
    ...
    callbacks = [Mycallback(model),
        ...
        ModelCheckpoint('%s/checkpoint/best_fine_tune_weights_%s2.h5'%(DPATH,chrom),monitor='val_loss',save_best_only=True,save_weights_only=True,mode='auto')]
...
def model_evaluation(batch_size):
    ...
    nb_regions = len(region_idx_train)
    for cell_idx in cell_idx_test:
        ...
        for i in range(nb_regions):
            ...
            X_test_mat[i,:,:,:] = X_seq[region_idx_train[i]]
            X_test_gexp[i,:] = tf_gexp_mat[cell_idx,:]
            #X_test_motif[i,:] = tf_motifscore_mat[i,:]
            X_test_motif[i,:] = tf_motifscore_mat[region_idx_train[i],:]
            #Y_test[i] = readscount_mat[i,cell_idx]
            Y_test[i] = readscount_mat[region_idx_train[i],cell_idx]
        ...
        f_out = open('%s/log/regression_%s2.log'%(DPATH,chrom),'a+')
        f_out.write('%d\t%.5f\n'%(cell_idx,pearsonr))
        f_out.close()
...
if  __name__ == "__main__" :
    ...
    chrom = sys.argv[1]
    #region_idx_train = [i for i,item in enumerate(readscount.index) if item.split(':')[0]==chrom]
    region_idx_train = list(range(len(readscount.index)))


    #for i in range(tf_motifscore.shape[0]):#i denotes region idx
    for i in region_idx_train:#i denotes region idx
    ...
    #parallel_model.load_weights('%s/checkpoint/best_fine_tune_weights.h5'%DPATH)
    parallel_model.load_weights('%s/checkpoint/best_fine_tune_weights_%s2.h5'%(DPATH,chrom))
    single_model = parallel_model.layers[-2]
    single_model.save('%s/checkpoint/single_model_%s2.h5'%(DPATH,chrom))
    model = load_model('%s/checkpoint/single_model_%s2.h5'%(DPATH,chrom))
