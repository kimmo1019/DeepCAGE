...
def model_evaluation(model,region_idx,padding=400):
    for cellid in test_cell_idx:
        ...
        f_out = open('%s/CNN/outcome/final/regression2_fold%d_seed%d_with_constaint_l20.01.log'%(DPATH,fold_idx,int(sys.argv[4])),'a+')
        f_out.write('%d\t%.5f\n'%(cellid,corr))
        f_out.close()
...
if  __name__ == "__main__" :
    ...
    model.save('%s/CNN/models/final/regression2_fold%d_seed%d_with_constaint_l20.01.h5'%(DPATH,fold_idx,int(sys.argv[4])))
