...
def model_evaluation(model,region_idx,padding=400):
    for cellid in test_cell_idx:
        ...
        hkl.dump([Y_test,Y_pred[:,0]],'%s/CNN/outcome/final/pre/regression_pre_%d_chr1.hkl'%(DPATH,cellid))
        f_out = open('%s/CNN/outcome/final/regression_fold%d_chr1.log'%(DPATH,fold_idx),'a+')
        f_out.write('%d\t%.5f\n'%(cellid,corr))
        f_out.close()
