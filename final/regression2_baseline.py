#set gexp to 0
#baseline 2 gexp to 0
#baseline 2.1 motif to 0 and gexp to 0
#baseline2.2    motif keep, gexp random
...
def model_evaluation(model,region_idx,padding=400):
    for cellid in test_cell_idx:
        ...
        f_out = open('%s/CNN/outcome/final/regression2.2_baseline_fold%d_seed%d_%s.log'%(DPATH,fold_idx,int(sys.argv[4]),str(ratio)),'a+')
        f_out.write('%d\t%.5f\n'%(cellid,corr))
        f_out.close()
...
if  __name__ == "__main__" :
    ...
    tf_gexp_norm = pd.DataFrame(data = np.random.rand(55,402),index=tf_gexp_log.index,columns=tf_gexp_log.columns)
    motif = pd.read_csv(motifscore_file,sep='\t',header=0,index_col=[0])
    #motif = pd.DataFrame(data = np.zeros((1435610, 402)),index=motif1.index,columns=motif1.columns)
    ...
    #model.save('%s/CNN/models/final/regression2.1_baseline_fold%d_seed%d_%s.h5'%(DPATH,fold_idx,int(sys.argv[4]),str(ratio)))
    model.save('%s/CNN/models/final/regression2.2_baseline_fold%d_seed%d_%s.h5'%(DPATH,fold_idx,int(sys.argv[4]),str(ratio)))
