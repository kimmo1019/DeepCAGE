...
def model_evaluation(model,region_idx,padding=400):
    for cellid in test_cell_idx:
        ...
        f_out = open('%s/CNN/outcome/final/classification_fold%d_seed%d_seq.log'%(DPATH,fold_idx,int(sys.argv[4])),'a+')
        f_out.write('%d\t%.5f\t%.5f\n'%(cellid,auROC,auPR))
        f_out.close()
...
if  __name__ == "__main__" :
    ...
    label_file='%s/processed_RNA_DNase/newdata/union.peaks.train.labels'%DPATH
    ...
    model.save('%s/CNN/models/final/classification_fold%d_seed%d_seq.h5'%(DPATH,fold_idx,int(sys.argv[4])))
