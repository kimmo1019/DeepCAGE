...
def model_training(model,region_idx,batch_size=128,epochs=100,steps_per_epoch = 10000):
    ...
    training_generator = generate_batch_data(readscount,tf_gexp_norm,motif,train_cell_idx, training_queue,batch_size=batch_size,padding=400,bootstrap=True)
...
def model_evaluation(model,region_idx,padding=400):
    for cellid in test_cell_idx:
        ...
        f_out = open('%s/CNN/outcome/final/DenseAttNet_seq_bootstrap_reg_fold%d.log'%(DPATH,fold_idx),'a+')
        f_out.write('%d\t%.5f\n'%(cellid,corr))
        f_out.close()
...
if  __name__ == "__main__" :
    ...
    readscount_file='%s/processed_RNA_DNase/union.peaks.readscount'%DPATH
    ...
    readscount = pd.read_csv(readscount_file,header=0,index_col=[0])
    ...
    model.save('%s/CNN/models/DenseAttnet_seq_bootstrap_reg_fold%d.h5'%(DPATH,fold_idx))
