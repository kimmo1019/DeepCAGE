...
def model_evaluation(model,region_idx,padding=400):
    for cellid in test_cell_idx:
        ...
        f_out = open('%s/CNN/outcome/final/gexp_trans/regression_fold%d_seed%d_part%d_%s.log'%(DPATH,fold_idx,int(sys.argv[4]),part_idx,str(ratio)),'a+')
        f_out.write('%d\t%.5f\n'%(cellid,corr))
        f_out.close()
...
if  __name__ == "__main__" :
    ...
    all_idx = list(range(readscount.shape[0]))
    random.shuffle(all_idx)
    part_idx = int(sys.argv[5])#1-10
    part_nb = int(len(all_idx)*ratio)
    region_idx = all_idx[(part_idx-1)*part_nb:part_idx*part_nb]
    model = DenseAttNet()
    model_training(model,region_idx,batch_size=128,epochs=100,steps_per_epoch = 5000)
    #t = time.strftime("%m-%d-%H", time.localtime()) 
    model.save('%s/CNN/models/final/gexp_trans/regression_fold%d_seed%d_part%d_%s.h5'%(DPATH,fold_idx,int(sys.argv[4]),part_idx,str(ratio)))
