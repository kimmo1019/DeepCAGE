...
def generate_batch_data(label,tf_gexp_norm,motif,train_cell_idx,training_queue,batch_size=128,padding=400,bootstrap=True):
    ...
    if bootstrap:
        while True:
            ...
            for i in range(len(present_queue)):
                ...
                Y_batch[i] = label.iloc[region_idx].loc[str(cellid)]
...
def generate_validation_data(label,tf_gexp_norm,motif, test_cell_idx, region_idx, padding=400):
    ...
    for cellid in test_cell_idx:
        for i in range(len(region_sample_idx)):
            info = label.index[region_sample_idx[i]]
            chrom,start,end = info.split(':')[0],int(info.split(':')[1].split('-')[0])-padding,int(info.split(':')[1].split('-')[1])+padding
            sequence = genome[chrom][start:end]
            motif_idx = chrom+':'+str(start)+'-'+str(end)
            X_valid_mat[idx,:,:,0] = seq_to_mat(sequence)
            X_valid_gexp[idx,:] = tf_gexp_norm.loc[cellid].values.reshape((1,-1))
            X_valid_motif[idx,:] = motif.loc[motif_idx].values.reshape((1,-1)) if motif_idx in motif.index else np.zeros((1,motif.shape[1]))
            Y_valid[idx] = label.iloc[region_sample_idx[i]].loc[str(cellid)]
            idx+=1
...
def model_training(model,region_idx,batch_size=128,epochs=100,steps_per_epoch = 10000):
    ...
    X_valid,Y_valid = generate_validation_data(label,tf_gexp_norm,motif,test_cell_idx,region_idx,padding=400)
    callbacks = [roc_callback(validation_data=(X_valid,Y_valid),patience=5)]
    #callbacks = [EarlyStopping(monitor='val_loss', patience=5, verbose=1)]
    training_generator = generate_batch_data(label,tf_gexp_norm,motif,train_cell_idx, training_queue,batch_size=batch_size,padding=400,bootstrap=False)
...
def model_evaluation(model,region_idx,padding=400):
    for cellid in test_cell_idx:
        ...
        for i in range(len(region_idx)):
            ...
            X_test_motif[i,:] = motif.loc[motif_idx].values.reshape((1,-1)) if motif_idx in motif.index else np.zeros((1,motif.shape[1]))
        ...
        f_out = open('%s/CNN/outcome/final/DenseAttNet_seq_gexp_motif_bootstrap_fold%d.log'%(DPATH,fold_idx),'a+')
        f_out.write('%d\t%.5f\t%.5f\n'%(cellid,auROC,auPR))
        f_out.close()
...
if  __name__ == "__main__" :
    ...
    motifscore_file = '%s/processed_RNA_DNase/union.peaks.motifscores.txt' %DPATH
    tf_gexp = pd.read_csv(tf_gexp_file ,sep='\t',header=0,index_col=[0])
    tf_gexp_log = np.log(tf_gexp+1)
    tf_gexp_norm = quantile_norm(tf_gexp_log)
    ...
    model.save('%s/CNN/models/final/DenseAttNet_seq_gexp_motif_bootstrap_fold%d.h5'%(DPATH,fold_idx))
