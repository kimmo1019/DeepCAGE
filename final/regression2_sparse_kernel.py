...
class roc_callback(Callback):
    ...
    def on_train_begin(self, logs={}):
        self.model.layers[133].set_weights([np.eye(402)*0.1,])
        self.wait = 0
        self.stopped_epoch = 0
        self.best = -np.Inf
        return
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        weights = [np.clip(w, -0.1, 0.1) for w in self.model.layers[133].get_weights()]
        self.model.layers[133].set_weights(weights)        
...
def l1_reg(weight_matrix):
    return 0.001 * (K.sum(K.abs(weight_matrix))-K.sum(K.abs(tf.diag_part(weight_matrix))))
...
def DenseAttNet():
    ...
    att_vec = Dense(402,use_bias=False, name='attention_vec',kernel_regularizer = l1_reg)(gexp_input)
    x1 = Multiply()([att_vec,gexp_input])
    x1 = Multiply()([x1,motif_input])
    x = Concatenate()([x,x1])
    x = Dense(512)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
...
def model_evaluation(model,region_idx,padding=400):
    for cellid in test_cell_idx:
        ...
        f_out = open('%s/CNN/outcome/final/regression2_fold%d_seed%d_%s_sparse_kernel0.001.log'%(DPATH,fold_idx,int(sys.argv[4]),str(ratio)),'a+')
        f_out.write('%d\t%.5f\n'%(cellid,corr))
        f_out.close()
...
if  __name__ == "__main__" :
    ...
    model.save('%s/CNN/models/final/regression2_fold%d_seed%d_%s_sparse_kernel0.001.h5'%(DPATH,fold_idx,int(sys.argv[4]),str(ratio)))
