...
class roc_callback(Callback):
    ...
    def on_train_begin(self, logs={}):
        #diag_kernel
        self.model.layers[134].set_weights([np.eye(402)*0.1,np.zeros(402)])
        #self.model.layers[134][1].set_weights(np.zeros(402))
        self.wait = 0
        self.stopped_epoch = 0
        self.best = -np.Inf
        return
    ...
    def on_epoch_end(self, epoch, logs={}):
        #weight clip
        weights = [np.clip(w, -0.1, 0.1) for w in self.model.layers[134].get_weights()]
        self.model.layers[134].set_weights(weights)
...
def model_evaluation(model,region_idx,padding=400):
    for cellid in test_cell_idx:
        ...
        f_out = open('%s/CNN/outcome/final/regression2_fold%d_seed%d_%s_diag_kernel_init.log'%(DPATH,fold_idx,int(sys.argv[4]),str(ratio)),'a+')
        f_out.write('%d\t%.5f\n'%(cellid,corr))
        f_out.close()
...
if  __name__ == "__main__" :
    ...
    model.save('%s/CNN/models/final/regression2_fold%d_seed%d_%s_diag_kernel_init.h5'%(DPATH,fold_idx,int(sys.argv[4]),str(ratio)))
